    ...
    <div className="w-full container max-w-[1420px] mx-auto py-24  flex  items-center gap-24 ">
      ...
      <div className="w-full flex-1">
        ...
        <div>
          <form action="" className="flex flex-col gap-4">
            ...
            <div className="w-full flex flex-col gap-2">
             ...
             <CommonInput
                ...
                id={"confirm_password"}
              />
              <span onClick={() => setConfirmPassShow(!confirmPassShow)} className="absolute cursor-pointer top-1/2 right-5 -translate-y-1/2">{confirmPassShow ? <Eye size={20}/> : <EyeOff size={20} /> } </span>
             </div>
            </div>
          ...
          <div className="flex items-center gap-1 justify-center pt-4">Already have an account? <Link to='/login'><h4 className="font-medium text-primary hover:underline">Log in</h4></Link></div>
        </div>
      </div>
    </div>
  );
